...
: mxWindow (window, 0, 0, 0, 0)
{
	...
	playerModels = new PlayerModels (this);

	setLoadEntirePAK (false);

	tab = new mxTab (this, 0, 0, 0, 0);
	tab->add (playerModels, "Player Models");
	tab->add (tvPAK, "PAK");
...
PAKViewer::handleEvent (mxEvent *event)
{
	...
	case mxEvent::Action:
	{
		...
		case IDC_PAKVIEWER: // tvPAK
			...
			{
				OnPAKViewer ();
				bool isMd2 = strstr (d_currLumpName, ".md2") != 0;
				bool isPcx = strstr (d_currLumpName, ".pcx") != 0;
				bool isTga = strstr (d_currLumpName, ".tga") != 0;
				pmMenu->setEnabled (1, isMd2 || isPcx);
				pmMenu->setEnabled (2, isMd2 || isPcx);
				...
				case 2:
					if (isMd2)
						...
						OnLoadTexture (ret == 1 ? TEXTURE_MODEL:TEXTURE_WEAPON);
					break;
			...
			else if (event->flags & mxEvent::DoubleClicked)
			{
				...
				if (mode == 0)
				{
					...
					if (!pak_ExtractFile (d_pakFile, d_currLumpName, str))
						mxMessageBox (this, "Error extracting from PAK file.", "MD2 Viewer", MX_MB_OK | MX_MB_ERROR);
					else
					{
						if (program)
						{
							...
							if ((int) WinExec (path, SW_SHOW) <= 32)
								mxMessageBox (this, "Error executing specified program.", "MD2 Viewer", MX_MB_OK | MX_MB_ERROR);
						}
					}
				}
				...
				else if (mode == 1)
				{
					...
					if (!pak_ExtractFile (d_pakFile, d_currLumpName, str))
						...
						if ((int) ShellExecute ((HWND) getHandle (), "open", str, 0, 0, SW_SHOW) <= 32)
							mxMessageBox (this, "Error executing document with associated program.", "MD2 Viewer", MX_MB_OK | MX_MB_ERROR);
				}
				...
				if (mode == 2)
				{
					if (!mx_strcasecmp (e, ".md2"))
						OnLoadModel (0);

					else if (!mx_strcasecmp (e, ".pcx"))
						OnLoadTexture (TEXTURE_MODEL);

					else if (!mx_strcasecmp (e, ".tga"))
...
PAKViewer::OnLoadModel (int pos)
{
	...
	bool isMD2 = (!mx_strcasecmp (mx_getextension (d_currLumpName), ".md2"));
	char prefix[16];


	if (isMD2)
		sprintf (prefix, "%s%d", "md2.md2", pos);
	else
		sprintf (prefix, "%s%d", "md2.pcx", pos);
	...
	if (isMD2)
	{
		md2_model_t *model = g_md2Viewer->glw->loadModel (str2, pos);
		if (model)
		{
			if (pos == 0)
				g_md2Viewer->initAnimation (model, -1);
			g_md2Viewer->setModelInfo (model, pos);
			...
			if (model->header.numSkins > 0)
			{
				...
				{
					if (g_md2Viewer->glw->loadTexture (str2, pos == 0 ? TEXTURE_MODEL:TEXTURE_WEAPON))
					{
			...
			mxMessageBox (this, "Error loading model.", "MD2 Viewer", MX_MB_OK | MX_MB_ERROR);
	}
	else // must be skin file
	{
		if (g_md2Viewer->glw->loadTexture (str2, pos == 0 ? TEXTURE_MODEL:TEXTURE_WEAPON))
		{
			...
			mxMessageBox (this, "Error loading skin.", "MD2 Viewer", MX_MB_OK | MX_MB_ERROR);
	}
...
PAKViewer::OnLoadTexture (int pos)
{
	...
	sprintf (prefix, "%d%s", pos, isTarga ? ".tga":".pcx");
	_makeTempFileName (str2, prefix);
	...
	// now load the things
	if (g_md2Viewer->glw->loadTexture (str2, pos))
	{
		if (pos == TEXTURE_BACKGROUND)
		{
			g_md2Viewer->cbBackground->setChecked (true);
			g_md2Viewer->glw->setFlag (F_BACKGROUND, true);
		}
		else if (pos == TEXTURE_WATER)
		{
			g_md2Viewer->cbWater->setChecked (true);
			g_md2Viewer->glw->setFlag (F_WATER, true);
		}
		g_md2Viewer->setRenderMode (3);
		g_md2Viewer->glw->redraw ();
	}
